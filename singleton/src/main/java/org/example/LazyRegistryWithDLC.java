package org.example;

/**
 * This class demonstrates singleton pattern using Double Check Locking or "classic" singleton.
 * This is also a lazy initializing singleton
 * Although this implementation solves the multi-threading issue with lazy initialization using volatile
 * and double check locking, the volatile keyword is guaranteed to work only after JVMs starting with version 1.5 and later
*/
public class LazyRegistryWithDLC {
    private volatile static LazyRegistryWithDLC instance;
    private LazyRegistryWithDLC() {}
    public static LazyRegistryWithDLC getInstance() {
        if (instance == null) {
            synchronized (LazyRegistryWithDLC.class) {
                if (instance == null) {
                    instance = new LazyRegistryWithDLC();
                }
            }
        }
        return instance;
    }
}
